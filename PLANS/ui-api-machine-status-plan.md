...
The UI implementation must also satisfy the production quality bar in
`docs/ui-ux-quality.md`. In practice this means the Control view is not done
until it remains stable under live SSE/poll updates, keeps machine-action
feedback local and explicit, preserves keyboard/focus behavior, uses a coherent
spacing/control sizing contract, and adapts at breakpoints without accidental
toolbar wrapping or control relocation.
...
- **Production UI quality gate:** Apply `docs/ui-ux-quality.md` to the Control and Files tabs. Group controls by operator task, keep global toolbars structurally stable, reserve fixed/bounded space for live status and validation, keep peer controls at consistent heights, provide accessible names/focus states, and use deliberate overflow/drawer/panel behavior at constrained widths instead of wrapping production controls.
...
- Add UI quality checks for live-update stability: no toolbar/header row-count change after status, file, or control updates; no input value overwrite while focused/dirty; local pending and terminal feedback for machine-action controls; usable keyboard focus order and visible focus; long values truncate or scroll inside reserved slots rather than pushing controls.
