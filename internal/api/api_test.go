...
func TestWebUIServed(t *testing.T) {
	...
	if !strings.Contains(string(jsBody), `document.body.classList.toggle("has-status-message", visible)`) {
		t.Errorf("app.js does not reserve content space only while the bottom status bar is visible")
	}
